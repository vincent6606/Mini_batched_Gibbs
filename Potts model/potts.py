...
class PottsLattice(multiprocessing.Process):
    ...
    def __init__(self,q, inv_tempature, lam, epoch=1e6,initial_state='r', size=(100, 100), show=False):
...
if __name__ == "__main__":
    ...
    factors = [2,5,20]
    for i in (factors):
        p = PottsLattice(q,inv_tempature=11,lam=(i), epoch =args.epoch, initial_state="r", size=(args.size,args.size))
        p.start()
        print('start')
        tasks.append(p)
    ...
    with open('errors_{}_{}_{}.csv'.format("151020",args.size,args.epoch),'wb') as f:
        for i in errors:
